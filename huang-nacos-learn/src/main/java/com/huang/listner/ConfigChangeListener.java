...
public class ConfigChangeListener extends AbstractConfigChangeListener {
     ...
     */
    private final static Map<String, List<NacosConfigRefreshAnnotationPostProcess.FieldInstance>> placeholderValueTargetMap = new HashMap<>();
    ...
    private void refreshTargetObjectFieldValue(Map<String, Object> newConfigMap) {
        ...
        for (String key : configChangeItemMap.keySet()) {
            ...
            if (placeholderValueTargetMap.containsKey(item.getOldValue())) {
                List<NacosConfigRefreshAnnotationPostProcess.FieldInstance> fieldInstances = placeholderValueTargetMap.get(item.getOldValue());
    ...
    private void addFieldInstance(String key, Field field, Object bean) {
        List<NacosConfigRefreshAnnotationPostProcess.FieldInstance> fieldInstances = placeholderValueTargetMap.get(key);
        if (CollectionUtils.isEmpty(fieldInstances)) {
            fieldInstances = new ArrayList<>();
        }
        fieldInstances.add(new NacosConfigRefreshAnnotationPostProcess.FieldInstance(bean, field));
    ...
    private void updateFieldValue(String key, String newValue, String oldValue) {
        List<NacosConfigRefreshAnnotationPostProcess.FieldInstance> fieldInstances = placeholderValueTargetMap.get(key);
        for (NacosConfigRefreshAnnotationPostProcess.FieldInstance instance : fieldInstances) {
            try {
